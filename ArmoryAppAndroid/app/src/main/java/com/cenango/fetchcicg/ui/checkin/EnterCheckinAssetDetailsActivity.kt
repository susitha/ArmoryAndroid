...
/**
 * Kotlin/Android equivalent of iOS's `EnterCheckinAssetDetailsViewController`
 * (Controllers/Checkin/EnterCheckinAssetDetailsViewController.swift). Same
 * shape as Checkout's equivalent screen (fetch asset, User/Password/Mags/
 * Weight), minus the assigned-user prefill — iOS doesn't do that here either,
 * since checkin isn't assumed to be by the same person the asset was
 * checked out to.
 */
class EnterCheckinAssetDetailsActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_enter_checkin_asset_details)
    ...
    private fun fetchAsset() {
        lifecycleScope.launch {
            when (val result = safeApiCall { app.api.getAsset(tag) }) {
                ...
                is ApiResult.Error -> {
                    AlertDialog.Builder(this@EnterCheckinAssetDetailsActivity)
     ...
     * privilege to check in and the logged-in user isn't in that group
    ...
    private fun checkAccessLevel(loadedAsset: Asset) {
        ...
        if (currentUser.userGroup.id != 1) {
            AlertDialog.Builder(this)
                .setTitle(R.string.checkin_access_denied_title)
                .setMessage(R.string.checkin_access_denied_message)
                .setPositiveButton(R.string.dialog_ok) { _, _ -> finish() }
                .setCancelable(false)
                .show()
        }
    ...
    private fun onNext() {
        ...
        startActivity(
            Intent(this, CheckinAssetDetailsSummaryActivity::class.java)
                .putExtra(CheckinAssetDetailsSummaryActivity.EXTRA_ASSET_ID, currentAsset.id)
                .putExtra(CheckinAssetDetailsSummaryActivity.EXTRA_ASSET_NAME, currentAsset.name)
                .putExtra(CheckinAssetDetailsSummaryActivity.EXTRA_ASSET_DESCRIPTION, currentAsset.description)
                .putExtra(CheckinAssetDetailsSummaryActivity.EXTRA_ASSET_SERIAL_NUMBER, currentAsset.serialNumber)
                .putExtra(CheckinAssetDetailsSummaryActivity.EXTRA_ASSET_CATEGORY_NAME, currentAsset.category.name)
                .putExtra(CheckinAssetDetailsSummaryActivity.EXTRA_USER_ID, user.id)
                .putExtra(CheckinAssetDetailsSummaryActivity.EXTRA_PASSWORD, passwordInput.text.toString())
                .putExtra(CheckinAssetDetailsSummaryActivity.EXTRA_MAGAZINES_COUNT, magsInput.text.toString().toIntOrNull() ?: -1)
                .putExtra(CheckinAssetDetailsSummaryActivity.EXTRA_TOTAL_WEIGHT, weightInput.text.toString())
        )
    }
}
